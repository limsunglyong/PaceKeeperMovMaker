(function () {
  ...
  const state = {
    ...
    bpm: 0,
    bpmOv: { enabled: true, x: 0.88, y: 0.13, color: "#ff4d5e", offset: 0 },
  ...
  function renderFrame(t) {
    ...
    if (state.bpmOv.enabled && state.bpm > 0) drawBpm(t);
    const trackOrder = new Map(state.tracks.map((track, index) => [track.id, index]));
    state.subs.slice().sort((a, b) => (trackOrder.get(a.trackId || "overlay-1") || 0) - (trackOrder.get(b.trackId || "overlay-1") || 0)).forEach((s) => {
      if (t >= s.start && t <= s.end) drawSub(s, fx);
    });
  }
  ...
  function drawBpm(t) {
    ...
    ctx.globalAlpha = 0.9;
    ctx.beginPath();
    ctx.arc(cx - 72, cy, 13 + pulse * 16, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 0.22 + pulse * 0.35;
    ctx.beginPath();
    ctx.arc(cx - 72, cy, 25 + pulse * 25, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 1;
    ctx.fillStyle = "#fff";
    ctx.font = "800 46px Consolas, monospace";
    ctx.fillText(String(state.bpm), cx + 4, cy - 2);
  ...
  function drawSub(s, fx) {
    ...
    if (s.type === "logo" && s.img && s.img.complete) {
      ...
      ctx.font = `800 ${s.size}px Inter, Segoe UI, sans-serif`;
      ctx.textAlign = align;
      ctx.textBaseline = "middle";
      const lines = wrapText(s.text, CW * 0.82);
      const lh = s.size * 1.18;
      let y = s.y * CH - (lines.length - 1) * lh / 2;
      const x = s.x * CW;
      lines.forEach((line) => {
        ...
        if (s.background || fx.background) {
          ...
          ctx.fillRect(bgX - 18, y - s.size * 0.68, metrics.width + 36, s.size * 1.28);
        }
        ...
        ctx.lineWidth = effect === "outline" ? s.size * 0.18 : s.size * 0.1;
        ctx.strokeStyle = effect === "none" ? "rgba(0,0,0,.7)" : "rgba(0,0,0,.9)";
  ...
  async function loadAudio(file, detect) {
    ...
    if (detect) {
      ...
      setStatus(`Loaded audio "${file.name}". Detected about ${state.bpm} BPM.`);
    } else {
  ...
  function addSubtitle() {
    ...
    const end = start + 3;
    state.subs.push({ id, type: "text", text: "New subtitle", start, end, trackId: activeOverlayTrackId(), x: 0.5, y: 0.82, size: 56, color: "#ffffff", background: true });
    state.selected = id;
    state.time = start;
    syncMedia(state.time);
    refresh();
  ...
  function addLogo(file) {
    ...
    img.onload = () => {
      const id = `s${Date.now()}`;
      const start = state.time;
      state.subs.push({ id, type: "logo", text: file.name, start, end: start + 6, trackId: activeOverlayTrackId(), x: 0.5, y: 0.18, size: 0.22, color: "#fff", url, img, blob: file });
      state.selected = id;
      state.time = start;
      syncMedia(state.time);
      refresh();
  ...
  function renderInspector() {
    ...
    if (selectedTrack && selectedTrack.type === "video") {
      ...
      el.inspector.innerHTML = `
        ...
        ${sub.type === "logo" ? slider("Logo size","sub.size",sub.size,.05,.6,.01,"%") : slider("Font size","sub.size",sub.size,20,120,1,"px")}
  ...
  function renderTimeline() {
    ...
    state.tracks.forEach((track) => {
      ...
      if (track.type === "video" && state.video && track.id === "video") {
      ...
      } else if (track.type === "audio" && state.audio && track.id === "audio") {
  ...
  function renderBpmLane(lane) {
    ...
    if (state.bpm > 0) {
      ...
      for (let t = state.audioOffset + state.bpmOv.offset; t <= duration(); t += beat) {
        if (t < 0) continue;
      ...
      tag.textContent = `${state.bpm} BPM - ${beat.toFixed(2)}s/beat`;
  ...
  function projectRecord(includeBlobs) {
    return {
      ...
      subs: state.subs.map((s) => ({ id: s.id, type: s.type, text: s.text, start: s.start, end: s.end, trackId: s.trackId, x: s.x, y: s.y, size: s.size, color: s.color, effect: s.effect, align: s.align, background: s.background })),
  ...
  async function loadProject(id) {
    ...
    el.bpmInput.value = state.bpm ? String(state.bpm) : "";
    if (rec.audioBlob) {
      await loadAudio(rec.audioBlob, rec.audioName || "audio", false);
      state.audioOffset = rec.audioOffset ?? state.audioOffset;
    }
    if (rec.videoBlob) {
      await loadVideo(rec.videoBlob);
      state.trim = rec.trim || state.trim;
      state.videoOffset = rec.videoOffset ?? state.videoOffset;
    }
    state.subs = (rec.subs || []).map((s) => {
      const out = { ...s };
      if (!out.trackId) out.trackId = overlayTracks()[0] ? overlayTracks()[0].id : addOverlayTrack();
  ...
  async function loadProjectJsonFile(file) {
    ...
    state.bpm = rec.bpm || 0;
    state.bpmOv = rec.bpmOv || state.bpmOv;
    state.subtitleFx = rec.subtitleFx || state.subtitleFx;
    state.subs = (rec.subs || []).map((sub) => ({ ...sub, trackId: sub.trackId || activeOverlayTrackId() }));
    state.selected = null;
    el.bpmInput.value = state.bpm ? String(state.bpm) : "";
  ...
  function wire() {
    ...
    $("newProjectBtn").onclick = () => {
      stop();
      state.id = null; state.video = null; state.audio = null; state.tracks = defaultTracks(); state.videoOffset = 0; state.audioOffset = 0; state.videoAudio = { muted: true, volume: 0.8 }; state.musicAudio = { muted: false, volume: 1 }; state.subtitleFx = { effect: "none", shadow: true, background: false, align: "center" }; state.subs = []; state.selected = null; state.bpm = 0; state.time = 0; state.trim = { start: 0, end: 0 };
    ...
    el.videoInput.onchange = (e) => e.target.files[0] && loadVideo(e.target.files[0]);
    el.audioInput.onchange = (e) => e.target.files[0] && loadAudio(e.target.files[0], true);
    ...
    el.bpmInput.oninput = (e) => { state.bpm = parseFloat(e.target.value) || 0; refresh(); };
